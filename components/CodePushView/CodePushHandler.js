...
const decorator = (options) => (WrappedComponent) => {
        ...
        render() {
          ...
          <Modal animationType="slide" transparent={true} visible={this.state.modalVisible} onRequestClose={() => {
            ...
            <TouchableOpacity activeOpacity={1} style={{ backgroundColor: 'rgba(1,1,1,0.5)', flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              ...
              <UpdateView width={D_WIDTH * 0.9} message={updateInfo.desc} versionName={updateInfo.label} progress={this.state.progress} packageSizeDesc={(updateInfo.packageSize / 1024 / 1024).toFixed(1) + 'MB'} isMandatory={updateInfo.isMandatory || this.state.isMandatory} progressDesc={this.state.progressDesc} onDownload={() => __awaiter(this, void 0, void 0, function* () {
                this.downloadAndInstall();
            })} onIgnore={() => {
                this.setState({
                    modalVisible: false,
    ...
    //传递默认属性
    if (options) {
        HOC.defaultProps.checkFrequency = options.checkFrequency;
        HOC.defaultProps.onlyDownloadWhenWifi = options.onlyDownloadWhenWifi;
        HOC.defaultProps.isDebugMode = options.isDebugMode;
    }
    return HOC;
};
